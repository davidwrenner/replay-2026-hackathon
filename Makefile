...
.PHONY: api test-api research test-research test-research-half test-injection ui ui-install temporal help
...
help:
	...
	@echo "  make test-research-half  - Test research endpoint with 5 data sources"
...
# Test research endpoint with 5 data sources (half)
test-research-half:
	@echo "Testing research endpoint with 5 data sources..."
	@curl -s -X POST http://localhost:8080/research \
		-H "Content-Type: application/json" \
		-d '{"dataSources":[{"name":"bloomberg"},{"name":"nytimes"},{"name":"polymarket"},{"name":"twitter"},{"name":"youtube"}],"prompt":"What is the market outlook for AI stocks?","riskTolerance":"medium","maxBudget":"1000"}' | jq .
